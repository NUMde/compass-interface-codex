...
class ValidationServerBundleBuilder(author: Organization, app: Device, questionnaire: Questionnaire) :
    ...
    init {
        ...
        bundle.addEntry(Bundle.BundleEntryComponent().apply {
            ...
            resource =
                SummaryNote(DateTimeType.now(), Reference(authorId), Reference(appId), Reference(questionnaireId))
        })
        bundle.addEntry(Bundle.BundleEntryComponent().apply {
    ...
    private val listOfGeccoResources: MutableList<Reference> =
        (bundle.entry.first().resource as Composition).section.last().entry
